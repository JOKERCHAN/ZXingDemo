...
public final class ViewfinderView extends View {
    ...
    //扫描线宽度
    private final int scanner_line_height;
    //边角的宽
    ...
    public ViewfinderView(Context context, AttributeSet attrs) {
        ...
        //默认为16sp
        labelTextSize = array.getDimensionPixelSize(R.styleable.ViewfinderView_label_text_size, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, 16, getResources().getDisplayMetrics()));
        //默认为1dp
        scanner_line_height = array.getDimensionPixelSize(R.styleable.ViewfinderView_scanner_line_height, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP,1, getResources().getDisplayMetrics()));
        //默认为4dp
        corner_rect_width = array.getDimensionPixelSize(R.styleable.ViewfinderView_corner_rect_width, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP,4, getResources().getDisplayMetrics()));
        //默认为10dp
        corner_rect_height = array.getDimensionPixelSize(R.styleable.ViewfinderView_corner_rect_height, (int) TypedValue.applyDimension(
    ...
    private void drawLaserScanner(Canvas canvas, Rect frame) {
        ...
        if (scannerStart <= scannerEnd) {
            RectF rectF = new RectF(frame.left + scanner_line_height, scannerStart, frame.right - scanner_line_height, scannerStart + scanner_line_height);
            canvas.drawOval(rectF, paint);
            scannerStart += scanner_line_height*2;//移动速度为line高度的2倍
        } else {
            scannerStart = frame.top;
        }
